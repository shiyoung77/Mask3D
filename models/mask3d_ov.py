...
class FFNLayer(nn.Module):
    ...
    def __init__(self, embed_dim, dim_feedforward=2048, dropout=0.0, activation="relu", normalize_before=False):
        super().__init__()
...
class CrossAttentionLayer(nn.Module):
    ...
    def forward_post(self, tgt, memory, memory_mask=None, memory_key_padding_mask=None, pos=None, query_pos=None):
        tgt2 = self.multi_head_attn(query=self.with_pos_embed(tgt, query_pos),
...
class Mask3D(nn.Module):
    def __init__(self, config, hidden_dim, num_queries, num_heads, dim_feedforward,
                 sample_sizes, shared_decoder, num_classes, num_decoders, dropout, pre_norm,
                 positional_encoding_type, non_parametric_queries, train_on_segments, normalize_pos_enc,
                 use_level_embed, scatter_type, hlevels, use_np_features,
                 voxel_size, max_sample_size, random_queries, gauss_scale, random_query_both, random_normal
                 ):
        super().__init__()
        ...
        self.num_levels = len(self.hlevels)  # 4, [0,1,2,3]
        ...
        for i, hlevel in enumerate(self.hlevels):
            tmp_cross_attention.append(
                CrossAttentionLayer(embed_dim=self.mask_dim, num_heads=self.num_heads, dropout=self.dropout,
                                    normalize_before=self.pre_norm)
            )
            tmp_squeeze_attention.append(nn.Linear(sizes[hlevel], self.mask_dim))
            tmp_self_attention.append(
                SelfAttentionLayer(embed_dim=self.mask_dim, num_heads=self.num_heads, dropout=self.dropout,
                                   normalize_before=self.pre_norm)
            )
            tmp_ffn_attention.append(
                FFNLayer(embed_dim=self.mask_dim, dim_feedforward=dim_feedforward, dropout=self.dropout,
                         normalize_before=self.pre_norm)
            )
            ...
            for i, hlevel in enumerate(self.hlevels):
                ...
                for k in range(len(decomposed_aux)):
                    ...
                    if pcd_size <= curr_sample_size:
                        # we do not need to sample, take all points and pad the rest with zeroes and mask it
                        ...
                        # we have more points in pcd as we like to, sample take a subset (no padding or masking needed)
